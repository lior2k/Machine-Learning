...
def read_file() -> [Point]:
    ...
    for line in file:
        data = line.split(' ')
        points_set.append(Point(float(data[0]), float(data[1]), int(data[2])))
    return points_set
...
def get_dist(p1: Point, p2: Point, p: int):
    if p == math.inf:
        return max(abs(p1.get_x() - p2.get_x()), abs(p1.get_y() - p2.get_y()))
    if p == 1:
        return abs(p1.get_x() - p2.get_x()) + abs(p1.get_y() - p2.get_y())
    return math.pow(math.pow(p1.get_x() - p2.get_x(), 2) + math.pow(p1.get_y() - p2.get_y(), 2), 1/2)
